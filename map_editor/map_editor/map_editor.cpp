// map_editor.cpp : 응용 프로그램에 대한 진입점을 정의합니다.
//
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MAP_EDITOR));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		if (SUCCEEDED(hr))
		{  			
			pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Yellow),&pYellowBrush); 
		}
		pRT->BeginDraw();
		...
		pRT->FillRectangle(D2D1::RectF(rc.left + 100.0f,rc.top + 100.0f,rc.right - 100.0f,rc.bottom - 100.0f), pYellowBrush);
		hr = pRT->EndDraw();	
		SafeRelease(&pRT);
